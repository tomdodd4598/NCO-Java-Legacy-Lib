...
public abstract class TileItemFluidGeneratorLegacy extends TileEnergyFluidSidedInventory implements IItemFluidGeneratorLegacy {
	...
	public TileItemFluidGeneratorLegacy(String name, int itemInSize, int fluidInSize, int itemOutSize, int fluidOutSize, int otherSize, @Nonnull List<ItemSorption> itemSorptions, @Nonnull IntList fluidCapacity, @Nonnull List<TankSorption> tankSorptions, List<Set<String>> allowedFluids, int capacity, @Nonnull BasicRecipeHandler recipeHandler) {
		...
		for (int i = 0; i < fluidInSize; ++i) {
			consumedTanks.add(new Tank(fluidCapacity.getInt(i), new HashSet<>()));
		}
	...
	public boolean isItemValidForSlot(int slot, ItemStack stack) {
		...
		return smart_processor_input ? recipeHandler.isValidItemInput(stack, slot, getItemInputs(false), getFluidInputs(false), recipeInfo) : recipeHandler.isValidItemInput(stack, slot);
	}
	...
	public void clearAllSlots() {
		IItemFluidGeneratorLegacy.super.clearAllSlots();
		consumedStacks.replaceAll(ignored -> ItemStack.EMPTY);
