...
public class TileItemProcessorLegacy extends TileEnergySidedInventory implements IItemProcessorLegacy, ITileSideConfigGuiLegacy<EnergyProcessorUpdatePacket>, IUpgradableLegacy {
	...
	public TileItemProcessorLegacy(String name, int itemInSize, int itemOutSize, @Nonnull List<ItemSorption> itemSorptions, double time, double power, boolean shouldLoseProgress, BiFunction<Double, Double, Long> capacityFunction, @Nonnull BasicRecipeHandler recipeHandler, int processorID, int sideConfigXOffset, int sideConfigYOffset) {
		this(name, itemInSize, itemOutSize, itemSorptions, time, power, shouldLoseProgress, true, capacityFunction, recipeHandler, processorID, sideConfigXOffset, sideConfigYOffset);
	}
	
	public TileItemProcessorLegacy(String name, int itemInSize, int itemOutSize, @Nonnull List<ItemSorption> itemSorptions, double time, double power, boolean shouldLoseProgress, boolean upgrades, BiFunction<Double, Double, Long> capacityFunction, @Nonnull BasicRecipeHandler recipeHandler, int processorID, int sideConfigXOffset, int sideConfigYOffset) {
		super(name, itemInSize + itemOutSize + (upgrades ? 2 : 0), ITileInventory.inventoryConnectionAll(itemSorptions), capacityFunction.apply(1D, 1D), power != 0 ? ITileEnergy.energyConnectionAll(EnergyConnection.IN) : ITileEnergy.energyConnectionAll(EnergyConnection.NON));
	...
	public void setCapacityFromSpeed() {
		long capacity = capacityFunction.apply(getSpeedMultiplier(), getPowerMultiplier());
	...
	public boolean isItemValidForSlot(int slot, ItemStack stack) {
		...
		return smart_processor_input ? recipeHandler.isValidItemInput(stack, slot, getItemInputs(), new ArrayList<>(), recipeInfo) : recipeHandler.isValidItemInput(stack, slot);
	}
	...
	public EnergyProcessorUpdatePacket getTileUpdatePacket() {
		return new EnergyProcessorUpdatePacket(pos, isProcessing, time, baseProcessTime, new ArrayList<>(), baseProcessPower, getEnergyStored());
	}
