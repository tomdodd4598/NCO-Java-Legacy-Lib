...
public class TileFluidProcessorLegacy extends TileEnergyFluidSidedInventory implements IFluidProcessorLegacy, ITileSideConfigGuiLegacy<EnergyProcessorUpdatePacket>, IUpgradableLegacy {
	...
	public TileFluidProcessorLegacy(String name, int fluidInSize, int fluidOutSize, @Nonnull List<ItemSorption> itemSorptions, @Nonnull IntList fluidCapacity, @Nonnull List<TankSorption> tankSorptions, List<Set<String>> allowedFluids, double time, double power, boolean shouldLoseProgress, boolean upgrades, BiFunction<Double, Double, Long> capacityFunction, @Nonnull BasicRecipeHandler recipeHandler, int processorID, int sideConfigXOffset, int sideConfigYOffset) {
		super(name, upgrades ? 2 : 0, ITileInventory.inventoryConnectionAll(itemSorptions), capacityFunction.apply(1D, 1D), power != 0 ? ITileEnergy.energyConnectionAll(EnergyConnection.IN) : ITileEnergy.energyConnectionAll(EnergyConnection.NON), fluidCapacity, allowedFluids, ITileFluid.fluidConnectionAll(tankSorptions));
	...
	public EnergyProcessorUpdatePacket getTileUpdatePacket() {
		return new EnergyProcessorUpdatePacket(pos, isProcessing, time, baseProcessTime, getTanks(), baseProcessPower, getEnergyStored());
	}
	...
	public void onTileUpdatePacket(EnergyProcessorUpdatePacket message) {
		...
		for (int i = 0, len = getTanks().size(); i < len; ++i) {
			getTanks().get(i).readInfo(message.tankInfos.get(i));
		}
	}
