...
public class ContainerItemFluidProcessorLegacy<PROCESSOR extends IItemFluidProcessorLegacy & ITileGuiLegacy<?>> extends ContainerTileLegacy<PROCESSOR> {
	...
	public ContainerItemFluidProcessorLegacy(EntityPlayer player, PROCESSOR tileEntity, BasicRecipeHandler recipeHandler) {
		super(tileEntity);
		tile = tileEntity;
	...
	public ItemStack transferStackInSlot(EntityPlayer player, int index) {
		...
		final boolean hasUpgrades = tile instanceof IUpgradableLegacy && ((IUpgradableLegacy) tile).hasUpgrades();
		int upgrades = hasUpgrades ? ((IUpgradableLegacy) tile).getNumberOfUpgrades() : 0;
