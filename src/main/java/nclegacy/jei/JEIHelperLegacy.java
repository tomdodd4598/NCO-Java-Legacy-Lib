...
public class JEIHelperLegacy {
	
	public static <WRAPPER extends JEIBasicRecipeWrapperLegacy<WRAPPER>> List<WRAPPER> getJEIRecipes(IGuiHelper guiHelper, IJEIHandlerLegacy<WRAPPER> jeiHandler, BasicRecipeHandler recipeHandler, Class<? extends WRAPPER> recipeWrapper) {
		ArrayList<WRAPPER> recipes = new ArrayList<>();
		if (recipeHandler != null) {
			for (BasicRecipe recipe : recipeHandler.getRecipeList()) {
				try {
					recipes.add(recipeWrapper.getConstructor(IGuiHelper.class, IJEIHandlerLegacy.class, BasicRecipeHandler.class, BasicRecipe.class).newInstance(guiHelper, jeiHandler, recipeHandler, recipe));
				}
				catch (Exception e) {
